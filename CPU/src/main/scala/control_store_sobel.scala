...
class control_store_sobel extends Module{
	val io = IO(new Bundle{
        ...
        //for sobel
        val SOBEL_MODE_CHANGE = Output(UInt(1.W))
	})
	...
	switch(stateReg){
		...
		is(stage1){
			...
			//for sobel
			io.SOBEL_MODE_CHANGE := Mux(instruction(6, 0) === "b1000011".U, 1.U, 0.U)
		}
		is(stage2){
			//setting ALU signals
...
object control_store_sobel extends App{
    (new chisel3.stage.ChiselStage).emitVerilog(new control_store_sobel())
}
