...
class data_path_sobel extends Module{
	...
	val registerFile = Module(new register_file_sobel())
	...
	registerFile.io.SOBEL_MODE_CHANGE := io.SOBEL_MODE_CHANGE
	registerFile.io.STORE_DATA_IN := io.STORE_DATA_IN
...
object data_path_sobel extends App{
    (new chisel3.stage.ChiselStage).emitVerilog(new data_path_sobel())
}
